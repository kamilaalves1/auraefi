...
test.describe('gateway Offline Harness', () => {
  test('capabilities expose gateway state dir/config in offline test mode', async ({ request }) => {
    ...
    expect(body.gatewayHome).toBe(true)
    expect(Boolean(body.claudeHome)).toBeTruthy()
    expect(Boolean(body.gateway)).toBe(EXPECT_GATEWAY)
  })

  test('sessions API reads fixture sessions without gateway install', async ({ request }) => {
  ...
  test('gateway config API reads fixture config', async ({ request }) => {
    ...
    expect(body.path.endsWith('gateway.json')).toBe(true)
