...
- Gateway startup and runtime checks now better tolerate read-only configs, missing `Vertex_HOME`, and container health-probe requirements.
...
- Docker/runtime regressions around Compose, assets, probes, `Vertex_HOME`, and read-only configs
...
- Vertex spawn compatibility when agents use configured default models
