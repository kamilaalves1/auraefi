...
### Dual-mode operation
- Mission Control now supports both gateway mode and local workstation mode cleanly.
...
- Gateway observability was added across sessions, memory, tasks, cron, and transcripts.
- Vertex coverage was expanded across channels, chat, sessions, cron, devices, approvals, logs, usage, and config editing.
...
- Vertex doctor warnings, doctor fix, and state-integrity handling are now surfaced directly in the UI.
...
- Agent/workspace creation and deletion flows were hardened end to end, including agent config normalization and cleanup.
...
- Local mode and gateway mode were hardened into first-class operating modes.
...
- Mission Control can register itself as the dashboard for an Vertex runtime without the earlier device-auth and dashboard URL failure cases.
...
- Session navigation was unified into Chat, with local Claude/External sessions included alongside gateway sessions.
- Gateway sessions, transcripts, memory, tasks, and cron surfaced throughout the app.
- Session transcript handling improved across local, Gateway, aggregated gateway, and disk-backed Vertex transcript sources.
...
- Task dispatch now polls assigned tasks and runs agents through Vertex with session linkage.
...
- Local agent discovery now covers `~/.agents`, `~/.external/agents`, and Claude agent roots more reliably.
...
- Security scan severity scoring, infrastructure checks, Vertex hardening checks, autofix behavior, and agent-side scan APIs improved.
...
- Branding assets and logos for Mission Control, Vertex, Claude, External, and Gateway were refreshed across the interface.
...
- Vertex doctor drift handling, doctor fix, config normalization, and workspace lifecycle cleanup were tightened to better support real self-hosted installs.

### E2E, CI, and release readiness
- The E2E harness now uses isolated Vertex runtime state, isolated skill roots, deterministic scheduler behavior, and safer env handling.
...
- Vertex backup creation and restore-oriented backup directory handling
...
- Vertex update-now trigger from the UI
- Approvals overlay, channels parity, chat parity, usage parity, device parity, and logs parity with the newer Vertex surface
...
- If you run Vertex, use the in-app doctor and security panels after upgrade to catch any stale config drift.
