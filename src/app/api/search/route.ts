...
export async function GET(request: NextRequest) {
  ...
  if (!typeFilter || typeFilter === 'task') {
    try {
      ...
      `, [workspaceId, likeQ, likeQ, likeQ, limit]) as any[]
      for (const t of tasks) {
        results.push({
          type: 'task',
          id: t.id,
          title: t.title,
          subtitle: `${t.status} ${t.assigned_to ? `Â· ${t.assigned_to}` : ''}`,
  ...
  if (!typeFilter || typeFilter === 'activity') {
    try {
      const activities = await dbGetAll(`
        ...
        ORDER BY created_at DESC LIMIT ?
      `, [workspaceId, likeQ, likeQ, limit]) as any[]
  ...
  // Search audit log (admin-only â€” audit_log is instance-global)
  if ((!typeFilter || typeFilter === 'audit') && auth.user.role === 'admin') {
    try {
      const audits = await dbGetAll(`
        ...
        ORDER BY created_at DESC LIMIT ?
      `, [likeQ, likeQ, likeQ, limit]) as any[]
    ...
    try {
      ...
      for (const m of messages) {
        results.push({
          type: 'message',
          id: m.id,
          title: `${m.from_agent} â†’ ${m.to_agent || 'all'}`,
