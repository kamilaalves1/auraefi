...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const loginFailures = authEventsQuery.filter((e: any) => e.event_type === 'auth.failure').length
    const tokenRotations = authEventsQuery.filter((e: any) => e.event_type === 'auth.token_rotation').length
    const accessDenials = authEventsQuery.filter((e: any) => e.event_type === 'auth.access_denied').length
    ...
    const timeline = await dbGetAll<any>(timelineQuery, timelineParams)
