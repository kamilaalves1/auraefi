...
/**
 * POST /api/connect â€” Register a direct CLI connection
 *
...
export async function POST(request: NextRequest) {
  ...
  let agent = await dbGet('SELECT * FROM agents WHERE name = ? AND workspace_id = ?', [agent_name, workspaceId]) as any
  if (!agent) {
    const result = await dbRun(`INSERT INTO agents (name, role, status, created_at, updated_at, workspace_id)
       VALUES (?, ?, 'online', ?, ?, ?)`, [agent_name, agent_role || 'cli', now, now, workspaceId])
    ...
    // Set agent online
    await dbRun('UPDATE agents SET status = ?, updated_at = ? WHERE id = ? AND workspace_id = ?', ['online', now, agent.id, workspaceId])
  ...
  await dbRun(`UPDATE direct_connections SET status = 'disconnected', updated_at = ? WHERE agent_id = ? AND status = 'connected'`, [now, agent.id])

  // Create new connection
  const connectionId = randomUUID()
  await dbRun(`INSERT INTO direct_connections (agent_id, tool_name, tool_version, connection_id, status, last_heartbeat, metadata, created_at, updated_at, workspace_id)
     VALUES (?, ?, ?, ?, 'connected', ?, ?, ?, ?, ?)`, [agent.id, tool_name, tool_version || null, connectionId, now, metadata ? JSON.stringify(metadata) : null, now, now, workspaceId])
...
/**
 * GET /api/connect â€” List all direct connections
 */
...
/**
 * DELETE /api/connect â€” Disconnect by connection_id
 */
export async function DELETE(request: NextRequest) {
  ...
  `, [connection_id, workspaceId]) as any
  if (!conn) {
    return NextResponse.json({ error: 'Connection not found' }, { status: 404 })
  }

  await dbRun('UPDATE direct_connections SET status = ?, updated_at = ? WHERE connection_id = ?', ['disconnected', now, connection_id])

  // Check if agent has other active connections; if not, set offline
  const otherActive = await dbGet('SELECT COUNT(*) as count FROM direct_connections WHERE agent_id = ? AND status = ? AND connection_id != ?', [conn.agent_id, 'connected', connection_id]) as any
  if (!otherActive?.count) {
    await dbRun('UPDATE agents SET status = ?, updated_at = ? WHERE id = ? AND workspace_id = ?', ['offline', now, conn.agent_id, workspaceId])
  }

  const agent = await dbGet('SELECT name FROM agents WHERE id = ? AND workspace_id = ?', [conn.agent_id, workspaceId]) as any
