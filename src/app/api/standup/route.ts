...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const agents = await dbGetAll<any>(agentQuery, agentParams);

    // Generate standup data for each agent in parallel
    const standupData = await Promise.all(agents.map(async (agent: any) => {
      const [completedTasks, inProgressTasks, assignedTasks, reviewTasks, blockedTasks, activityCount, commentsToday] = await Promise.all([
        ...
        `, [agent.name, workspaceId, startOfDay, endOfDay]),
        dbGetAll(`
      ...
      return {
        ...
        activity: {
          actionCount: activityCount?.count ?? 0,
          commentsCount: commentsToday?.count ?? 0,
        }
      };
    }));
    ...
    const teamAccomplishments = standupData
      .flatMap(agent => agent.completedToday.map((task: any) => ({ ...task, agent: agent.agent.name })))
      .sort((a: any, b: any) => b.updated_at - a.updated_at);

    const teamBlockers = standupData
      .flatMap(agent => agent.blocked.map((task: any) => ({ ...task, agent: agent.agent.name })))
      .sort((a: any, b: any) => {
    ...
    await dbRun(`
      REPLACE INTO standup_reports (date, report, created_at, workspace_id)
      VALUES (?, ?, ?, ?)
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const standupRows = await dbGetAll<{ date: string; report: string; created_at: number }>(`
    ...
    const countRow = await dbGetOne<{ total: number }>(
