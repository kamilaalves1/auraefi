...
export async function PUT(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const existing = await dbGetOne<any>('SELECT * FROM clients WHERE id = ? AND workspace_id = ?', [Number(id), workspaceId])
    ...
    await dbRun(
      'UPDATE clients SET name = ?, description = ?, updated_at = UNIX_TIMESTAMP() WHERE id = ? AND workspace_id = ?',
      [name, description, Number(id), workspaceId]
    )

    const row = await dbGetOne('SELECT * FROM clients WHERE id = ?', [Number(id)])
