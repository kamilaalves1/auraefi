...
export async function GET(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const pipeline = await dbGetOne('SELECT id FROM work_pipelines WHERE id = ? AND workspace_id = ?', [Number(id), workspaceId])
...
export async function PUT(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    await dbTransaction(async (conn) => {
      await conn.execute(
        'DELETE FROM pipeline_columns WHERE pipeline_id = ? AND workspace_id = ?',
        [Number(id), workspaceId]
      )
    ...
    const saved = await dbGetAll<any>(
      `SELECT pc.*, a.name as agent_name
       FROM pipeline_columns pc
       LEFT JOIN agents a ON a.id = pc.agent_id
       WHERE pc.pipeline_id = ? AND pc.workspace_id = ?
