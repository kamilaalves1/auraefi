...
export async function DELETE(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const pipeline = await dbGet('SELECT id, provider FROM work_pipelines WHERE id = ? AND workspace_id = ?', [Number(id), workspaceId]) as { id: number; provider: string } | undefined
    if (!pipeline) return NextResponse.json({ error: 'Pipeline not found' }, { status: 404 })

    const result = await dbRun(`UPDATE pipeline_card_runs SET status = 'cancelled', updated_at = UNIX_TIMESTAMP()
       WHERE id = ? AND workspace_id = ? AND status IN ('failed', 'running', 'waiting_input')`, [Number(runId), workspaceId])
...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    await dbRun(`UPDATE pipeline_card_runs SET status = 'cancelled', updated_at = UNIX_TIMESTAMP()
       WHERE id = ? AND workspace_id = ?`, [Number(runId), workspaceId])
