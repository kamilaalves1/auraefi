...
export async function GET(request: NextRequest) {
  ...
  try {
    const completed = (await readUserOnboardingSetting(ONBOARDING_SETTING_KEYS.completed, auth.user.username)) === 'true'
    const skipped = (await readUserOnboardingSetting(ONBOARDING_SETTING_KEYS.skipped, auth.user.username)) === 'true'
    const checklistDismissed = (await readUserOnboardingSetting(ONBOARDING_SETTING_KEYS.checklistDismissed, auth.user.username)) === 'true'
    const completedStepsRaw = await readUserOnboardingSetting(ONBOARDING_SETTING_KEYS.completedSteps, auth.user.username)
