...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Scan message for injection â€” this gets forwarded directly to an agent
    ...
    const agent = await dbGet('SELECT * FROM agents WHERE name = ? AND workspace_id = ?', [to, workspaceId]) as any
    if (!agent) {
    ...
    // Gateway CLI delivery removed â€” log the request and create notification only
