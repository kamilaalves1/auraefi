...
export async function GET(request: NextRequest) {
  ...
  if (gateways.length === 0) {
    ...
    const seeded = await dbGetAll('SELECT * FROM gateways ORDER BY is_primary DESC, name ASC', []) as GatewayEntry[]
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      await dbRun('INSERT INTO audit_log (action, actor, detail) VALUES (?, ?, ?)', ['gateway_added', auth.user?.username || 'system', `Added gateway: ${name} (${host}:${port})${agentsRegistered ? `, registered ${agentsRegistered} agent(s)` : ''}`])
    } catch { /* audit might not exist */ }

    const gw = await dbGet('SELECT * FROM gateways WHERE id = ?', [result.insertId]) as GatewayEntry
...
export async function PUT(request: NextRequest) {
  ...
  const existing = await dbGet('SELECT * FROM gateways WHERE id = ?', [id]) as GatewayEntry | undefined
  ...
  const updated = await dbGet('SELECT * FROM gateways WHERE id = ?', [id]) as GatewayEntry
...
export async function DELETE(request: NextRequest) {
  ...
  const gw = await dbGet('SELECT * FROM gateways WHERE id = ?', [id]) as GatewayEntry | undefined
  if (gw?.is_primary) {
  ...
  try {
    await dbRun('INSERT INTO audit_log (action, actor, detail) VALUES (?, ?, ?)', ['gateway_removed', auth.user?.username || 'system', `Removed gateway: ${gw?.name}`])
  } catch { /* audit might not exist */ }
