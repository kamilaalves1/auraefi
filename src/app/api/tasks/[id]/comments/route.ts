...
export async function GET(
  ...
  try {
    ...
    // Verify task exists
    const task = await dbGet('SELECT id FROM tasks WHERE id = ? AND workspace_id = ?', [taskId, workspaceId]);
    if (!task) {
...
export async function POST(
  ...
  try {
    ...
    // Verify task exists
    const task = await dbGet<any>('SELECT * FROM tasks WHERE id = ? AND workspace_id = ?', [taskId, workspaceId]);
    if (!task) {
      return NextResponse.json({ error: 'Task not found' }, { status: 404 });
    }
    ...
    if (parent_id) {
      const parentComment = await dbGet('SELECT id FROM comments WHERE id = ? AND task_id = ? AND workspace_id = ?', [parent_id, taskId, workspaceId]);
      if (!parentComment) {
    ...
    const createdComment = (await dbGet<Comment>('SELECT * FROM comments WHERE id = ? AND workspace_id = ?', [commentId, workspaceId]))!;
