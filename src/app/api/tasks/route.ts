...
import { Task, db_helpers, dbGetOne, dbGetAll, dbTransaction } from '@/lib/db';
...
async function resolveProjectId(workspaceId: number, requestedProjectId?: number): Promise<number> {
  if (typeof requestedProjectId === 'number' && Number.isFinite(requestedProjectId)) {
    const project = await dbGetOne<{ id: number }>(`
      ...
      LIMIT 1
    `, [requestedProjectId, workspaceId])
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const tasks = await dbGetAll<Task>(query, params);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const mentionResolution = resolveMentionRecipients(description || '', workspaceId);
    ...
    const taskId = await dbTransaction(async (conn) => {
      await conn.execute(`
        UPDATE projects
      ...
      const [counterRows] = await conn.execute(`
        ...
        WHERE id = ? AND workspace_id = ?
      `, [resolvedProjectId, workspaceId])
      const row = (counterRows as any[])[0] as { ticket_counter: number } | undefined
    ...
    const createdTask = await dbGetOne<Task>(`
      SELECT t.*, p.name as project_name, p.ticket_prefix as project_prefix
      FROM tasks t
      LEFT JOIN projects p
        ON p.id = t.project_id AND p.workspace_id = t.workspace_id
    ...
    if (parsedTask.project_id) {
      const project = await dbGetOne<any>(`
        ...
        WHERE id = ? AND workspace_id = ?
      `, [parsedTask.project_id, workspaceId])
  ...
  try {
    ...
    await dbTransaction(async (conn) => {
      for (const task of tasks) {
        const [oldRows] = await conn.execute<any[]>('SELECT * FROM tasks WHERE id = ? AND workspace_id = ?', [task.id, workspaceId])
        const oldTask = (oldRows as any[])[0] as Task | undefined
        if (!oldTask) continue;

        if (task.status === 'done') {
          const [reviewRows] = await conn.execute<any[]>(`
            SELECT status FROM quality_reviews
            WHERE task_id = ? AND reviewer = 'aegis' AND workspace_id = ?
            ORDER BY created_at DESC
            LIMIT 1
          `, [task.id, workspaceId])
          const aegisApproved = (reviewRows as any[])[0]?.status === 'approved'
          if (!aegisApproved) {
            throw new Error(`Aegis approval required for task ${task.id}`)
          }
          await conn.execute(
            'UPDATE tasks SET status = ?, updated_at = ?, completed_at = COALESCE(completed_at, ?) WHERE id = ? AND workspace_id = ?',
