...
export async function GET(request: NextRequest) {
  ...
  try {
    const rules = await dbGetAll<AlertRule>(
...
export async function POST(request: NextRequest) {
  ...
  if (rawBody.action === 'evaluate') {
    return await evaluateRules(workspaceId)
  }
  ...
  try {
    ...
    const rule = await dbGetOne<AlertRule>(
      'SELECT * FROM alert_rules WHERE id = ? AND workspace_id = ?',
      [result.insertId, workspaceId]
    )
...
export async function PUT(request: NextRequest) {
  ...
  const existing = await dbGetOne<AlertRule>(
    'SELECT * FROM alert_rules WHERE id = ? AND workspace_id = ?',
    [id, workspaceId]
  )
  ...
  const updated = await dbGetOne<AlertRule>(
    'SELECT * FROM alert_rules WHERE id = ? AND workspace_id = ?',
    [id, workspaceId]
  )
 ...
 */
async function evaluateRules(workspaceId: number): Promise<NextResponse> {
  let rules: AlertRule[]
  try {
    rules = await dbGetAll<AlertRule>(
    ...
    if (triggered) {
      ...
      try {
        ...
        if (rule.action_type === 'webhook' && config.url) {
          ...
          `, ['coordinator', `📧 E-mail: ${rule.name}`, `Limite excedido. Envio para ${config.email_to} requer SMTP configurado.`, rule.id, workspaceId])
          ...
          await dbRun(`
            INSERT INTO notifications (recipient, type, title, message, source_type, source_id, workspace_id)
            VALUES (?, 'alert', ?, ?, 'alert_rule', ?, ?)
          `, [recipient, `🚫 Bloqueio ativo: ${rule.name}`, `Limite excedido — novos envios LLM bloqueados. Regra: "${rule.name}"`, rule.id, workspaceId])
        } else {
  ...
  return NextResponse.json({ evaluated: rules.length, triggered: triggeredCount, results })
}

async function evaluateRule(rule: AlertRule, now: number, workspaceId: number): Promise<boolean> {
  try {
    switch (rule.entity_type) {
      case 'agent': return await evaluateAgentRule(rule, now, workspaceId)
      case 'task': return await evaluateTaskRule(rule, now, workspaceId)
      case 'session': return await evaluateSessionRule(rule, now, workspaceId)
      case 'activity': return await evaluateActivityRule(rule, now, workspaceId)
      case 'token_cost': return await evaluateTokenCostRule(rule, now, workspaceId)
...
async function evaluateAgentRule(rule: AlertRule, now: number, workspaceId: number): Promise<boolean> {
  ...
  if (condition_operator === 'count_above' || condition_operator === 'count_below') {
    const row = await dbGetOne<any>(`SELECT COUNT(*) as c FROM agents WHERE workspace_id = ? AND ${safeColumn('agents', condition_field)} = ?`, [workspaceId, condition_value])
    const count = row?.c || 0
  ...
  if (condition_operator === 'age_minutes_above') {
    ...
    const row = await dbGetOne<any>(`SELECT COUNT(*) as c FROM agents WHERE workspace_id = ? AND status != 'offline' AND ${safeColumn('agents', condition_field)} < ?`, [workspaceId, threshold])
    const count = row?.c || 0
    return count > 0
  }

  const agents = await dbGetAll<any>(`SELECT ${safeColumn('agents', condition_field)} as val FROM agents WHERE workspace_id = ? AND status != 'offline'`, [workspaceId])
...
async function evaluateTaskRule(rule: AlertRule, _now: number, workspaceId: number): Promise<boolean> {
  ...
  if (condition_operator === 'count_above') {
    const row = await dbGetOne<any>(`SELECT COUNT(*) as c FROM tasks WHERE workspace_id = ? AND ${safeColumn('tasks', condition_field)} = ?`, [workspaceId, condition_value])
    const count = row?.c || 0
    return count > parseInt(condition_value)
  }

  if (condition_operator === 'count_below') {
    const row = await dbGetOne<any>(`SELECT COUNT(*) as c FROM tasks WHERE workspace_id = ?`, [workspaceId])
    const count = row?.c || 0
    return count < parseInt(condition_value)
  }

  const tasks = await dbGetAll<any>(`SELECT ${safeColumn('tasks', condition_field)} as val FROM tasks WHERE workspace_id = ?`, [workspaceId])
...
async function evaluateSessionRule(rule: AlertRule, _now: number, workspaceId: number): Promise<boolean> {
  ...
  if (condition_operator === 'count_above') {
    const row = await dbGetOne<any>(`SELECT COUNT(*) as c FROM agents WHERE workspace_id = ? AND status = 'busy'`, [workspaceId])
    const count = row?.c || 0
    return count > parseInt(condition_value)
  }
...
async function evaluateActivityRule(rule: AlertRule, now: number, workspaceId: number): Promise<boolean> {
  ...
  if (condition_operator === 'count_above') {
    ...
    const row = await dbGetOne<any>(`SELECT COUNT(*) as c FROM activities WHERE workspace_id = ? AND created_at > ? AND ${safeColumn('activities', condition_field)} = ?`, [workspaceId, hourAgo, condition_value])
    const count = row?.c || 0
    return count > parseInt(condition_value)
  }
...
async function evaluateTokenCostRule(rule: AlertRule, now: number, workspaceId: number): Promise<boolean> {
  ...
  if (condition_field === 'daily_cost_usd') {
    const row = await dbGetOne<{ total: number }>(
      `SELECT COALESCE(SUM(cost_usd), 0) as total FROM token_usage WHERE workspace_id = ? AND created_at > ?`,
      [workspaceId, dayAgo]
    )
    return compareValue(row?.total ?? 0, condition_operator, condition_value)
  }
