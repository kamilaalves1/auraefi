...
import { dbGetAll, dbGetOne, Activity } from '@/lib/db';
...
async function handleActivitiesRequest(request: NextRequest, workspaceId: number) {
  try {
    ...
    const activities = await dbGetAll<Activity>(query, params);
    ...
    const enhancedActivities = await Promise.all(activities.map(async activity => {
      ...
      try {
        switch (activity.entity_type) {
          case 'task': {
            const task = await dbGetOne<any>('SELECT id, title, status FROM tasks WHERE id = ? AND workspace_id = ?', [activity.entity_id, workspaceId]);
            if (task) {
          ...
          case 'agent': {
            const agent = await dbGetOne<any>('SELECT id, name, role, status FROM agents WHERE id = ? AND workspace_id = ?', [activity.entity_id, workspaceId]);
            if (agent) {
          ...
          case 'comment': {
            ...
            `, [activity.entity_id, workspaceId, workspaceId]);
            if (comment) {
              entityDetails = {
                type: 'comment',
                ...comment,
    ...
    const countResult = await dbGetOne<{ total: number }>(countQuery, countParams);
...
async function handleStatsRequest(request: NextRequest, workspaceId: number) {
  try {
    ...
    const activityStats = await dbGetAll<{ type: string; count: number }>(`
      SELECT
        type,
        COUNT(*) as count
      FROM activities
    ...
    const activeActors = await dbGetAll<{ actor: string; activity_count: number }>(`
      SELECT
        actor,
