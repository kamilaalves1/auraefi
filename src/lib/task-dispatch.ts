...
interface DispatchableTask {
  ...
  tags?: string[]
  /** LLM escolhido na atribuição da coluna do fluxo (lido do metadata da tarefa) */
...
async function getPipelineConfig(workspaceId: number): Promise<PipelineConfig> {
  try {
    ...
    const row = await dbGet('SELECT config_json FROM work_pipelines WHERE workspace_id = ? ORDER BY id ASC LIMIT 1', [workspaceId]) as { config_json: string | null } | undefined
    if (row?.config_json) {
      const parsed = JSON.parse(row.config_json)
      if (typeof parsed === 'object' && parsed !== null) return parsed
    }
    // Fallback: any pipeline in the system (tasks may have a different workspace_id
    // than the pipeline, e.g. when created manually or via inbox routing)
    const any = await dbGet('SELECT config_json FROM work_pipelines ORDER BY id ASC LIMIT 1', []) as { config_json: string | null } | undefined
    if (any?.config_json) {
      const parsed = JSON.parse(any.config_json)
      if (typeof parsed === 'object' && parsed !== null) return parsed
    }
    return {}
  } catch { return {} }
...
async function isGatewayAvailable(): Promise<boolean> {
  try {
    const row = await dbGet('SELECT url FROM gateways LIMIT 1', []) as { url: string } | undefined
    if (!row?.url) return false
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const net = require('net') as typeof import('net')
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const { URL: NodeURL } = require('url') as typeof import('url')
    const parsed = new NodeURL(row.url.startsWith('http') ? row.url : `http://${row.url}`)
    const host = parsed.hostname
    const port = parseInt(parsed.port || (parsed.protocol === 'https:' ? '443' : '80'), 10)
    return await new Promise<boolean>((resolve) => {
      const socket = net.createConnection({ host, port, timeout: 2000 }, () => { socket.destroy(); resolve(true) })
...
async function classifyDirectModel(task: DispatchableTask, pipelineCfg?: PipelineConfig): Promise<string> {
  // 0. Modelo definido na atribuição da coluna do fluxo — máxima prioridade
  ...
  // 4. Fallback model configured in the pipeline screen
  if (cfg.llm_fallback_model) return cfg.llm_fallback_model

  // 5. No model configured at all — throw a clear error pointing to the pipeline screen
  throw new Error('Nenhum modelo LLM configurado no pipeline. Configure os modelos na tela de Pipeline.')
}
...
export async function dispatchAssignedTasks(): Promise<{ ok: boolean; message: string }> {
  ...
  // Parse JSON tags column + extrai llm_model do metadata (gravado pelo pipeline-engine)
  for (const task of tasks) {
    if (typeof task.tags === 'string') {
      try { task.tags = JSON.parse(task.tags as string) } catch { task.tags = undefined }
    }
    try {
      const raw = (task as unknown as { metadata?: string }).metadata
      if (raw) {
