import { dbGetOne, dbGetAll, dbRun, dbTransaction, logAuditEvent } from './db'
...
async function runBackup(): Promise<{ ok: boolean; message: string }> {
  // Aurora MySQL backups are managed at the infrastructure level (RDS snapshots / mysqldump).
  // The SQLite file-copy approach does not apply to MySQL.
  logAuditEvent({
    action: 'auto_backup',
    actor: 'scheduler',
    detail: { note: 'Aurora MySQL — use RDS snapshots or mysqldump for backups' },
  }).catch(() => {})
  return { ok: true, message: 'Backup: Aurora MySQL — managed via RDS snapshots' }
}
  ...
  try {
    ...
    const staleAgents = await dbGetAll<{ id: number; name: string; status: string; last_seen: number | null }>(`
      ...
      WHERE status != 'offline'
        ...
        AND id NOT IN (
          SELECT DISTINCT CAST(jt.agent_id_val AS UNSIGNED)
          FROM pipeline_columns pc,
          JSON_TABLE(pc.assignments_json, '$[*]' COLUMNS (agent_id_val JSON PATH '$.agent_id')) AS jt
          WHERE jt.agent_id_val IS NOT NULL
        )
    `, [threshold])
    ...
    await dbTransaction(async (conn) => {
      for (const agent of staleAgents) {
        await conn.execute('UPDATE agents SET status = ?, updated_at = ? WHERE id = ?', ['offline', now, agent.id])
        await conn.execute(`
...
async function syncAgentLiveStatuses(): Promise<number> {
  ...
  await dbTransaction(async (conn) => {
    for (const agent of agents) {
      ...
      await conn.execute('UPDATE agents SET status = ?, last_seen = ?, last_activity = ?, updated_at = ? WHERE id = ?',
