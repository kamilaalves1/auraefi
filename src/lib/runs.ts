...
export async function getRun(id: string, workspaceId?: number): Promise<AgentRun | null> {
  const row = await dbGet<any>('SELECT * FROM runs WHERE id = ? AND workspace_id = ?', [id, workspaceId ?? 1])
...
export async function listRuns(opts: {
  ...
  const countRow = await dbGet<{ c: number }>(`SELECT COUNT(*) as c FROM runs ${where}`, params as any[])
  const total = countRow?.c ?? 0
  const rows = await dbGetAll<any>(`SELECT * FROM runs ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`, [...params, limit, offset] as any[])
