...
export async function getMentionTargets(workspaceId: number): Promise<MentionTarget[]> {
  ...
  const users = await dbGetAll<{ username: string; display_name?: string | null }>(`
  ...
  const agents = await dbGetAll<{ name: string; role?: string | null; config?: string | null }>(`
  ...
  for (const agent of agents) {
    ...
    const candidateHandles = [agentHandle, normalizeAgentHandle(recipient), recipient.toLowerCase()]
