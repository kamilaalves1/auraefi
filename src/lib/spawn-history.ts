...
export async function recordSpawnFinish(id: number, input: {
  ...
  try {
    ...
    const spawn = await dbGet<any>('SELECT session_id, agent_name, workspace_id FROM spawn_history WHERE id = ?', [id])
    let cost = undefined
    if (spawn?.session_id) {
      const tokens = await dbGet<any>(`
...
export async function getSpawnStats(opts?: {
  ...
  const byAgent = await dbGetAll<any>(`
    SELECT
      agent_name,
      COUNT(*) as count,
