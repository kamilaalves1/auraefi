...
describe('config data paths', () => {
  ...
  it('derives data, backup, and token paths from MISSION_CONTROL_DATA_DIR', async () => {
    ...
    expect(config.backupDir).toBe('/tmp/mission-control-data/backups')
    expect(config.tokensPath).toBe('/tmp/mission-control-data/mission-control-tokens.json')
  })

  it('respects explicit token path override', async () => {
    const config = await loadConfigWithEnv({
      MISSION_CONTROL_DATA_DIR: '/tmp/mission-control-data',
  ...
  it('uses a build-scoped worker data dir during next build', async () => {
    ...
    expect(config.backupDir).toMatch(/^\/tmp\/build-scratch\/worker-\d+\/backups$/)
    expect(config.tokensPath).toMatch(/^\/tmp\/build-scratch\/worker-\d+\/mission-control-tokens\.json$/)
  })

  it('prefers build-specific token override during next build', async () => {
    const config = await loadConfigWithEnv({
      NEXT_PHASE: 'phase-production-build',
      MISSION_CONTROL_DATA_DIR: '/tmp/runtime-data',
    ...
    expect(config.backupDir).toMatch(new RegExp(`^${expectedBuildRoot.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}/worker-\\d+/backups$`))
