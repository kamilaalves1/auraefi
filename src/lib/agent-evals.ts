...
export async function runDriftCheck(
  ...
  const [currentTokens, baselineTokens, currentTools, baselineTools, currentTasks, baselineTasks] =
    await Promise.all([
      dbGetOne<any>(`
        SELECT AVG(input_tokens + output_tokens) as avg_tokens
        FROM token_usage
        WHERE agent_name = ? AND created_at > ?
      `, [agentName, currentStart]),
      dbGetOne<any>(`
        SELECT AVG(input_tokens + output_tokens) as avg_tokens
        FROM token_usage
      ...
      dbGetOne<any>(`
        ...
        FROM mcp_call_log
        WHERE agent_name = ? AND workspace_id = ? AND created_at > ?
      `, [agentName, workspaceId, currentStart]),
      dbGetOne<any>(`
        SELECT
          COUNT(*) as total,
          SUM(CASE WHEN success = 1 THEN 1 ELSE 0 END) as successes
      ...
      `, [agentName, workspaceId, baselineStart, baselineEnd]),
      dbGetOne<any>(`
        SELECT
          COUNT(*) as total,
          SUM(CASE WHEN status = 'done' THEN 1 ELSE 0 END) as completed
...
export async function getDriftTimeline(
  ...
  for (let i = weeks - 1; i >= 0; i--) {
    ...
    const [tokens, tools, tasks] = await Promise.all([
      dbGetOne<any>(`
        SELECT AVG(input_tokens + output_tokens) as avg_tokens
        FROM token_usage
        WHERE agent_name = ? AND created_at > ? AND created_at <= ?
      ...
      dbGetOne<any>(`
        ...
        FROM mcp_call_log
        WHERE agent_name = ? AND workspace_id = ? AND created_at > ? AND created_at <= ?
      `, [agentName, workspaceId, weekStart, weekEnd]),
      dbGetOne<any>(`
        SELECT
          COUNT(*) as total,
          SUM(CASE WHEN status = 'done' THEN 1 ELSE 0 END) as completed
