...
import { dbGetAll, dbTransaction } from './db'
...
export async function syncSkillsFromDisk(): Promise<{ ok: boolean; message: string }> {
  try {
    ...
    const localSources = ['user-agents', 'project-agents', 'gateway', 'workspace']
    ...
    const dbRows = await dbGetAll<SkillRow>(
    ...
    await dbTransaction(async (conn) => {
      ...
      for (const [key, row] of dbMap) {
        if (!diskMap.has(key) && !row.registry_slug) {
          ...
          await conn.execute(`DELETE FROM skills WHERE source = ? AND name = ?`, [row.source, row.name])
          deleted++
        }
      }
    })
