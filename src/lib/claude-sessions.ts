...
import { dbRun, dbGetAll, dbTransaction } from './db'
...
export async function syncClaudeSessions(force = false): Promise<{ ok: boolean; message: string }> {
  ...
  try {
    ...
    await dbTransaction(async (conn) => {
      ...
      await conn.execute('UPDATE claude_sessions SET is_active = 0', [])
      ...
      const [allRowsResult] = await conn.execute('SELECT session_id FROM claude_sessions', [])
      const allRows = allRowsResult as Array<{ session_id: string }>
      for (const row of allRows) {
        if (!liveIds.has(row.session_id)) {
          await conn.execute('DELETE FROM claude_sessions WHERE session_id = ?', [row.session_id])
          removed++
        }
      }
    })
