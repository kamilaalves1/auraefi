...
const mockDbGetAll = vi.fn(() => Promise.resolve([]))
...
describe('evalTaskCompletion', () => {
  ...
  it('returns score based on completed/total ratio', async () => {
    mockDbGet.mockResolvedValue({ total: 10, completed: 7, successful: 5 })
  ...
  it('returns score 1.0 when no tasks exist', async () => {
    mockDbGet.mockResolvedValue({ total: 0, completed: 0, successful: 0 })
  ...
  it('fails when completion rate is below 70%', async () => {
    mockDbGet.mockResolvedValue({ total: 10, completed: 5, successful: 3 })
...
describe('evalCorrectnessScore', () => {
  ...
  it('returns success rate when no feedback ratings', async () => {
    mockDbGet.mockResolvedValue({ total: 10, successful: 8, avg_rating: null })
  ...
  it('blends success rate with feedback rating', async () => {
    mockDbGet.mockResolvedValue({ total: 10, successful: 10, avg_rating: 4.0 })
  ...
  it('fails when score is below 0.6', async () => {
    mockDbGet.mockResolvedValue({ total: 10, successful: 3, avg_rating: null })
