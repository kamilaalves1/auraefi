...
const DEFAULT_GATEWAY_CLIENT_ID = process.env.NEXT_PUBLIC_GATEWAY_CLIENT_ID || 'gateway-control-ui'
...
export function useWebSocket() {
  ...
  const getGatewayErrorHelp = useCallback((message: string): string => {
    ...
    if (
      ...
      return 'Gateway requires device identity. Open Vertex Control Center via HTTPS (or localhost), then reconnect so WebCrypto signing can run.'
    }
    ...
    if (normalized.includes('invalid connect params') || normalized.includes('/client/id')) {
      return 'Gateway rejected client identity params. Ensure NEXT_PUBLIC_GATEWAY_CLIENT_ID is set to gateway-control-ui and reconnect.'
    }
  ...
  const sendConnectHandshake = useCallback(async (ws: WebSocket, nonce?: string) => {
    ...
    const connectRequest = {
      ...
      params: {
        ...
        client: {
          id: clientId,
          displayName: 'Vertex Control Center',
          version: APP_VERSION,
          platform: 'web',
          mode: clientMode,
